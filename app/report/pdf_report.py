...
def build_report(pdf_path: str, case: dict, trace_row: dict, result: dict,
    ...
    for paragraph in [
        ...
        "(3) Bitcoin address clustering (common-input-ownership): where "
        "several addresses were spent together as inputs of one transaction "
        "they are reported as one presumed wallet, with the evidencing "
        "transaction(s) and a confidence level; CoinJoin-like transactions "
        "are excluded. Clusters are inferences (see the Address Clusters "
        "section when present). No other change-detection heuristic is "
        "applied.",
        ...
        "suspect's own change in the flow; clustering identifies such "
        "addresses only when they are later co-spent. (c) Funds entering "
        "smart contracts (swaps, bridges, "
